<!DOCTYPE html>
<html lang="es">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title><?php echo $__env->yieldContent('title'); ?></title>
    <link rel="icon" href="/images/favicon.ico"/>
    <link rel="shortcut icon" href="/images/favicon.ico" />
    <script src="https://ajax.googleapis.com/ajax/libs/jquery/3.3.1/jquery.min.js"></script>
   <script src="https://code.jquery.com/ui/1.12.1/jquery-ui.js"></script>
   <link rel="stylesheet" href="https://stackpath.bootstrapcdn.com/bootstrap/4.3.1/css/bootstrap.min.css" integrity="sha384-ggOyR0iXCbMQv3Xipma34MD+dH/1fQ784/j6cY/iJTQUOhcWr7x9JvoRxT2MZw1T" crossorigin="anonymous">
<script src="https://stackpath.bootstrapcdn.com/bootstrap/4.3.1/js/bootstrap.min.js" integrity="sha384-JjSmVgyd0p3pXB1rRibZUAYoIIy6OrQ6VrjIEaFf/nJGzIxFDsf4x0xIM+B07jRM" crossorigin="anonymous"></script>
<link href="/css/all.css" rel="stylesheet"> 
<link rel="stylesheet" href="/css/styles.css?id=<?php echo rand(1, 100000000);?>">
    
<script src="/js/custom.js?id=<?php echo rand(1, 10000000);?>"></script>   

    
</head>
<body>

    <div class="container_ms">

        <div class="menu-toggle">
            <button id="toggleButton">☰</button>
        </div>
        <aside class="sidebar_ms" id="sidebar_ms">
            <!-- Contenido del menú lateral -->
            <?php echo $__env->make('layouts.left_navigation', array_except(get_defined_vars(), array('__data', '__path')))->render(); ?>

    </aside>
        <main class="main-content">
        <?php echo $__env->make('layouts.navigation', array_except(get_defined_vars(), array('__data', '__path')))->render(); ?>
      
        <section class="content" id="main-content">
        <?php echo $__env->yieldContent('content'); ?>
      </section>
        </main>
    </div>
    <script>
document.getElementById('toggleButton').addEventListener('click', function() {
    var sidebar_ms = document.getElementById('sidebar_ms');
    if (sidebar_ms.classList.contains('open')) {
        sidebar_ms.classList.remove('open');
    } else {
        sidebar_ms.classList.add('open');
    }
});


    </script>
<script src="https://cdnjs.cloudflare.com/ajax/libs/popper.js/1.12.3/umd/popper.min.js" integrity="sha384-vFJXuSJphROIrBnz7yo7oB41mKfc8JzQZiCq4NCceLEaO4IHwicKwpJf9c9IpFgh" crossorigin="anonymous"></script>


<!-- IE10 viewport hack for Surface/desktop Windows 8 bug -->
<script src="/js/ie10-viewport-bug-workaround.js"></script>

<?php echo $__env->yieldContent('footerjs'); ?>
 <script src="/js/footerScripts.js?id=<?php echo rand(1, 10000000);?>"></script>
<link rel="stylesheet" type="text/css" href="<?php echo e(asset('css/nestable.css')); ?>">
<script type="text/javascript" src="<?php echo e(asset('js/jquery.nestable.js')); ?>"></script>
<script type="text/javascript" src="<?php echo e(asset('js/myseo.js')); ?>"></script>
</body>
</html>
