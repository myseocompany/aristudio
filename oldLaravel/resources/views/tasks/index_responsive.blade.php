...
@include('tasks.filter_responsive')
      ...
      <div class="form-group col-md-12">
        ...
        <button id="btn_edit_{{$item->id}}" onclick="updateTaskFromModal({{$item->id}});" class="btn btn-sm btn-primary my-2 my-sm-0">Update</button>
          ...
          <div class="card" style="width: 100%;">
            ...
            <div class="card-body" id="comment_{{$item->id}}" style="width: 100% !important;">
              <label for="description_message">Add Comment</label>
              <div class="input-group">
                  <input class="form-control" style="width: 45%;" type="text" id="description_modal_edit_{{$item->id}}" class="form-control">
              ...
              <div class="form-group">
                ...
                @foreach ($users as $user)
                    <option value="{{$user->id}}" 
                      @if ($user->id == $request->user_id) selected="selected" @endif>{{$user->name}}</option>
                @endforeach
                </select>
              </div>
...
//enviar formulario al dar enter
  ...
  document.getElementById('task_description').addEventListener('keydown', inputCharacters);
