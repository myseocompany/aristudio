    ...
    window.addEventListener('load', () => {
      ...
      const myConfig = {
        ...
        scaleX: {
          ...
          labels: [@for($i=0;$i< count($users_graph); $i++) '{{substr($users_graph[$i]->name, 0, 4)}}' @if($i <(count($users_graph)-1)) , @endif @endfor]
          ,
        },
        scaleY: {
        ...
        series: [
          @for($i=0; $i<$weeks_diff; $i++){
            ...
            values: [@for($j=0;$j< count($users_graph); $j++)   @if(isset($data[$j][$i])) {{$data[$j][$i]}} @else 0 @endif @if($j<(count($users_graph)-1)) , @endif @endfor],
            text: '{{"Week " . $i}}'
          }
          @if($i<($weeks_diff-1)) , @endif 
          @endfor
        ]
      };
