...
<!--
        ...
        -->
          ...
          @foreach(Auth::user()->getProjects() as $item)
            <li class="nav-item list-project-item" style="color:{{$item->color}}">
              <a class="nav-link" href="/tasks?project_id={{$item->id}}">
                {{$item->name}}</a>
            </li>
          @endforeach
        ...
        -->
      <!--module 4 users-->
       @if (  Auth::user()->getRoleModule(Auth::user()->role_id,4) == 4)
       ...
       @endif   
          ...
          @endif
            @if ( Auth::user()->getRoleModule(Auth::user()->role_id,7) == 7) <!-- module 7 = Role Module-->
          <li class="nav-item">
          ...
          @if (  Auth::user()->role_id == 2 || Auth::user()->role_id == 1)
      <!-- @if ( Auth::user()->getRoleModule(Auth::user()->role_id,5) == 5)-->
          <li class="nav-item">
...
<script type="text/javascript">
  $(document).ready(function(){
   $.ajax({
      type: 'GET',
      url: "/get_notifications/{{Auth::user()->id}}",
      dataType: 'json',
