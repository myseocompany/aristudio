    ...
    window.addEventListener('load', () => {
      ...
      const myConfig = {
        ...
        scaleX: {
          ...
          labels: [@for($i=0;$i< $weeks; $i++) '{{substr($weeks_array[$i][0],8,-8)}}' @if($i <($weeks-1)) , @endif @endfor]
          ,
        },
        scaleY: {
        ...
        series: [
          @for($j=0;$j< count($users_graph); $j++)
          
          {
            values: [@for($k=0;$k<$weeks;$k++){{$data[$k][$j]}}@if($k<($weeks-1)) , @endif  @endfor],
            text: '{{$users_graph[$j]->name}}',
            backgroundColor: colors[{{$j}}],

          } @if($j<count($users_graph)-1) , @endif 
