    ...
    window.addEventListener('load', () => {
      ...
      const myConfig = {
        ...
        scaleX: {
          ...
          labels: [@for($i=0;$i< count($users); $i++) @if( hasPoints($i, $time_span, $data)) '{{substr($users[$i]->name, 0, 8)}}' @if($i <(count($users)-1)) @endif,  @endif @endfor]
          ,
        },
        scaleY: {
        ...
        series: [
          ...
          
          {
            ...
            values: [@for($j=0;$j< count($users); $j++) @if( hasPoints($j, $time_span, $data)) {{$data[$j][$i]}}@if($j<(count($users)-1)) @endif,  @endif @endfor],
